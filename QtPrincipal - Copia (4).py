...
'''
def enviar_email():
    #Inserindo o arquivo com a tabela
    tabela_pedidos = QFileDialog.getOpenFileNames()
    print(tabela_pedidos)
    ...
    #def gerarRelatorio(self):
        # Selecionando o QLineEdit e armazenadno o texto dele nas variaveis
    #dataIni = str(window.lineEditDataIni.text())
    #dataFin = str(window.lineEditDataFin.text())
    dataIni = window.lineEditDataIni.text()
    dataFin = window.lineEditDataFin.text()
    print(dataIni, dataFin)
    print(tabela_pedidos)
    ...
    (tabela_pedidos['DATA'] >= dataIni) & (tabela_pedidos['DATA'] <= dataFin)]
